...
// RECORTE
	exports.fixCanvasPos = function(){
		...
		var fix = function(e){
		    ...
		    canvasTop = (de.clientHeight - window.innerHeight) / 2 - 40;
		};
...
define("scripts/game.js", function(exports){
	...
	var barbette = function(){
	    ...
	    var startX = random( window.innerWidth ), endX = random( window.innerWidth ), startY = window.innerHeight;
	...
	exports.sliceAt = function( fruit, angle ){
        ...
        // Increment score based on fruit type
        if (fruit.type === "apple") //preco
            scoreNumber += 0.05;

        else if (fruit.type === "sandia") 
            scoreNumber += 0.1;

        else if (fruit.type === "kiwi") 
          scoreNumber += 0.2;

        else if (fruit.type === "cereja") 
          scoreNumber += 0.25;
        ...
        else if (fruit.type === "abacaxi") 
          scoreNumber += 1;

        else if (fruit.type === "premiada") 
          scoreNumber += 100;
...
define("scripts/layer.js", function(exports){
	...
	exports.getLayer = function( name ){
		...
		if( p = layers[name] ){
			...
			p = layers[name] = Raphael( layer, '100vw', '100vh' );
			// if( Ucren.isSafari )
			//     p.safari();
			return p;
		}
...
define("scripts/main.js", function(exports){
	...
	exports.start = function(){
	    ...
	    log("Colocando o background");
      log("Colhendo as frutas");
      log("Afiando a faca");
      log("Preparando as bombas");
      log("Prepare-se!");
	    log.clear();
...
define("scripts/sence.js", function(exports){
  ...
  var gameStartSnd 
    ...
    gameStartSnd = sound.create( "sound/start" );
		...
		[ background, homeMask, logo, ninja, homeDesc, newGame, score, lose, gameOver, flash ].invoke( "set" );
	};
	...
	exports.showMenu = function( callback ){
      ...
      let sandiaLeftWidth = Number((window.innerWidth / 2) - 18),
          sandiaTopHeight = Number((window.innerHeight / 2) - 18)

	    sandia = fruit.create( "sandia", sandiaLeftWidth, sandiaTopHeight, true );
	
	    [ sandia ].forEach(function( f ){ f.isHomeMenu = 1; });
	    sandia.isNewGameIcon = 1//volta
...
define("scripts/factory/fruit.js", function(exports){
	...
	var dropTime = 900, dropXScope = 200, shadowPos = 50;//droptime
    ...
    premiada: [ "images/fruit/premiada.png", 70, 70, 32, -135, 0, "#F0D540" ]//premiada
	};

	var types = [ 'boom', 'apple', 'sandia', 'kiwi', 'cereja', 'banana', 'manga', 'limao', 'pessego', 'morango', 'laranja', 'abacaxi', 'premiada' ];
  ...
  // Preload the images and then use the preloaded images in your 'infos' object

  preloadImages(imageUrls)
    .then(images => {
      // images is an array of preloaded Image objects in the same order as imageUrls
      for (let i = 0; i < types.length; i++) {
        infos[types[i]][0] = images[i].src;
      }
    })
    .catch(error => {
  ...
  function preloadImages(urls) {
    ...
    // Loop through each image URL and create Image objects
...
preloadImages(imageUrls)
  .then(() => {
      // All images are preloaded, now you can use the preloaded images in 'infos'
      infos.boom[0] = "images/fruit/boom.png"; 

      infos.apple[0] = "images/fruit/apple.png"; 
      infos.sandia[0] = "images/fruit/sandia.png"
      infos.kiwi[0] = "images/fruit/kiwi.png"

      infos.cereja[0] = "images/fruit/cereja.png"; 
      infos.banana[0] = "images/fruit/banana.png"
      infos.manga[0] = "images/fruit/manga.png"

      infos.limao[0] = "images/fruit/limao.png"; 
      infos.pessego[0] = "images/fruit/pessego.png"
      infos.morango[0] = "images/fruit/morango.png"

      infos.laranja[0] = "images/fruit/laranja.png"; 
      infos.abacaxi[0] = "images/fruit/abacaxi.png"
      infos.premiada[0] = "images/fruit/premiada.png"
  })
  .catch(error => {
      console.error("Image preload failed:", error);
  });
	...
	ClassFruit.prototype.shotOut = function(){
	    ...
	    return function( start, endX ){
			...
			if( this.type != "boom" )
			 	this.rotate( 0, ( random( 180 ) + 90 ) * sign[ random( 200 ) ] );//mudei
	...
	function getType() {
    let randomNumber = random(13), // Generate a random number between 0 and 9,
        randomFruit = types[random(13)]

    if (randomNumber <= 4) return "boom"// 40% chance for "boom"
    
    else if (randomNumber <= 13) { // 60% chance for other results
      return randomFruit // Replace "types" with your actual array of types
    }
...
exports.set = function() {
  image = layer.createImage("default", "images/background.jpg", 0, 0, '100vw', '100vh');
}
...
define("scripts/object/game-over.js", function(exports){
	...
	exports.set = function(){
    let gameOverLeftWidth = Number((window.innerWidth / 2)-130),
        gameOverTopHeight = Number((window.innerHeight / 2)-50)

		this.image = layer.createImage( "default", "images/game-over.png", gameOverLeftWidth, gameOverTopHeight, 250, 60 ).hide().scale( 1e-5, 1e-5 );

    console.log(gameOverLeftWidth)
	};
...
define("scripts/object/home-desc.js", function(exports){
	...
	exports = displacement.create("images/home-desc.png", 141, 81, -161, 140, 7, 80, tween.exponential.co, 500);;
...
define("scripts/object/home-mask.js", function(exports){
	...
	exports = displacement.create("images/home-mask.png", window.innerWidth, 103, 0, -183, 0, 0, tween.exponential.co, 1e3);;
...
define("scripts/object/logo.js", function(exports){
	...
	exports = displacement.create("images/logo.png", 150, 80, 17, -182, 5, 0, tween.exponential.co, 1e3);;
...
define("scripts/object/lose.js", function(exports){
  ...
  let numberWidth = window.innerWidth
	
	var conf1 = { src: "images/x.png", sx: 650, ex: Number(numberWidth - 93), y: 6, w: 22, h: 19 };
	var conf2 = { src: "images/xx.png", sx: 671, ex: Number(numberWidth - 68), y: 6, w: 27, h: 26 };
	var conf3 = { src: "images/xxx.png", sx: 697, ex: Number(numberWidth - 37), y: 8, w: 31, h: 32 };
    ...
    // Define the condition for image replacement (e.g., when number is between 1 and 2)
    const shouldReplaceImage = number >= 0 && number <= 3;

    if (shouldReplaceImage) {
      const infx = inf[(++number - 1) % inf.length];

      const newImageSrc = infx[1].src.replace("x.png", "xf.png");

      infx[0].attr("src", newImageSrc);
...
define("scripts/object/new-game.js", function(exports){
  ...
  let containerLeftWidth = Number((window.innerWidth / 2) - 88),
      containerTopHeight = Number((window.innerHeight / 2) - 93)
	
	exports = rotate.create("images/new-game.png", containerLeftWidth, containerTopHeight, 160, 160, 1e-5, tween.exponential.co, 500);
  console.log(exports)
...
define("scripts/object/ninja.js", function(exports){
	...
	exports = displacement.create("images/ninja.png", 120, 66, 315, -140, 147, 19, {
...
define("scripts/object/score.js", function(exports){
	...
	exports.set = function(){
	    ...
	    text2 = layer.createText( "default", "Meta: R$ 10", text2Sx, 48, "#FBFBFB", "14px" ).hide();
	};
